...
# ======================================================================================
# This script proceed bias-correction for rx5day over Indonesia
# ======================================================================================
...
# Create a function to read in rx5day index
...
base_name = "*rx5day*.nc"
solar_name = "*G6solar*rx5day*.nc"
sulfur_name = "*G6sulfur*rx5day*.nc"
ssp245_name = "*ssp245*rx5day*.nc"
ssp585_name = "*ssp585*rx5day*.nc"

# Read in observation dataset
obs_da = open_ncfile(glob.glob(obspath+base_name)[0]).precip.sel(
...
for baseline,future in zip(all_bpath,all_fpath):
    ...
    base_da = open_ncfile(glob.glob(base_pattern)[0]).pr.sel(
        time=slice("1990","2014")) #print(base_file)
    
    solar_pattern = os.path.join(future, solar_name)
    solar_da = open_ncfile(glob.glob(solar_pattern)[0]).pr #print(solar_file)
    
    sulfur_pattern = os.path.join(future, sulfur_name)
    sulfur_da = open_ncfile(glob.glob(sulfur_pattern)[0]).pr #print(sulfur_file)
    
    ssp245_pattern = os.path.join(future, ssp245_name)
    ssp245_da = open_ncfile(glob.glob(ssp245_pattern)[0]).pr #print(ssp245_file)

    ssp585_pattern = os.path.join(future, ssp585_name)
    ssp585_da = open_ncfile(glob.glob(ssp585_pattern)[0]).pr #print(ssp585_file)
    ...
    bc_baseline_ds = bias_corrected_baseline.to_dataset(dim=None,
                                name='highest_five_day_precipitation_amount_per_time_period')
    ...
    bc_baseline_ds.to_netcdf(
        ...
        format = "NETCDF4",
        engine ="netcdf4",
        encoding= {"highest_five_day_precipitation_amount_per_time_period": {"dtype": "float32"}},
        unlimited_dims='time')
...
print("This is the end of bias-correction process for rx5day index")
# -----------------------------------------------------------------